...
public class Bird : MonoBehaviour
{
    ...
    void OnGroundBehaviors()
    {
        ...
        if (idle)
        {
            ...
            if (Random.value < Time.deltaTime * .33)
            {
                ...
                if (rand < .3)
                {
                    DisplayBehavior(Behaviors.sing);
                }
                else if (rand < .5)
                {
                    DisplayBehavior(Behaviors.peck);
                }
                else if (rand < .6)
                {
                    DisplayBehavior(Behaviors.preen);
                }
                else if (!perched && rand < .7)
                {
                    DisplayBehavior(Behaviors.ruffle);
                }
                else if (!perched && rand < .85)
                {
                    DisplayBehavior(Behaviors.hopForward);
                }
                else if (!perched && rand < .9)
                {
                    DisplayBehavior(Behaviors.hopLeft);
                }
                else if (!perched && rand < .95)
                {
                    DisplayBehavior(Behaviors.hopRight);
                }
                else if (!perched && rand <= 1)
                {
                    DisplayBehavior(Behaviors.hopBackward);
                }
                else
                {
                    DisplayBehavior(Behaviors.sing);
    ...
    void DisplayBehavior(Behaviors behavior)
    {
        idle = false;
        switch (behavior)
        {
