...
public class daynight_cycle1 : MonoBehaviour
{
    ...
    IEnumerator Breathing()
    {
        while (true)
        {
            StartCoroutine(StartFadeInhale(InhaleSource, inhaleTime + 0.1f, 0.0f));// starts the couroutine
            yield return new WaitForSeconds(inhaleTime + breathDelay); // Wait For the end of the coroutinea by delay
            StartCoroutine(StartFadeInhale(ExhaleSource, exhaleTime + 0.1f, 0.0f));// starts the coroutine withe different audiosource and time.
    ...
    {
        audioSource.volume = 0.7f;// sets audio source volume value
